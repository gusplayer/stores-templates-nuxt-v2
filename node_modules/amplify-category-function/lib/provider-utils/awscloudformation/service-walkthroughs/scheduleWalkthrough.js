...
async function scheduleWalkthrough(context, params) {
    const projectBackendDirPath = context.amplify.pathManager.getBackendDirPath();
    const resourceDirPath = path_1.default.join(projectBackendDirPath, categoryName, params.resourceName);
        ...
        if (await context.amplify.confirmPrompt.run('Do you want to invoke this function on a recurring schedule?', false)) {
            try {
                ...
                if (context.input.command === 'update') {
                    ...
                    cloudformationHelpers_1.constructCloudWatchEventComponent(cfnFilePath, cfnContent);
...
async function cronServiceWalkthrough(context) {
    ...
    switch (intervalAnswer.interval) {
        case "Minutes": {
            cloudwatchRule = cronHelper_1.minuteHelper(context);
            break;
        }
        case "Hourly": {
            cloudwatchRule = cronHelper_1.hourHelper(context);
            break;
        }
        case "Daily": {
            var exp = new cronBuilder_1.CronBuilder();
            exp = await cronHelper_1.timeHelper(exp);
            cloudwatchRule = exp.build();
            cloudwatchRule = 'cron(' + replaceAt(cloudwatchRule, cloudwatchRule.lastIndexOf('*'), '?') + ' ' + '*' + ')';
            break;
        }
        case "Weekly": {
            var exp1 = new cronBuilder_1.CronBuilder();
            exp1 = await cronHelper_1.weekHelper(exp1);
            exp1 = await cronHelper_1.timeHelper(exp1);
            cloudwatchRule = exp1.build();
            cloudwatchRule = 'cron(' + cloudwatchRule + ' ' + '*' + ')';
            break;
        }
        case "Monthly": {
            var exp2 = new cronBuilder_1.CronBuilder();
            exp2 = await cronHelper_1.monthHelper(exp2, context);
            exp2 = await cronHelper_1.timeHelper(exp2);
            cloudwatchRule = exp2.build();
            cloudwatchRule = 'cron(' + replaceAt(cloudwatchRule, cloudwatchRule.lastIndexOf('*'), '?') + ' ' + '*' + ')';
            break;
        }
        case "Yearly": {
            var exp3 = new cronBuilder_1.CronBuilder();
            exp3 = await cronHelper_1.yearHelper(exp3, context);
            exp3 = await cronHelper_1.timeHelper(exp3);
