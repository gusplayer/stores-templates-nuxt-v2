...
exports.askLayerSelection = async (layerMetadataFactory, amplifyMeta, runtimeValue, previousSelections = []) => {
    const lambdaLayers = [];
    const dependsOn = [];
    ...
    const layerOptions = lodash_1.default.keys(functionMeta)
        ...
        .filter(key => isRuntime(runtimeValue).inRuntimes(functionMeta[key].runtimes));
