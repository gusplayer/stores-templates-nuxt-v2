...
async function zipLayer(context, resource) {
    ...
    const resourcePath = path_1.default.join(context.amplify.pathManager.getBackendDirPath(), resource.category, layerName);
    ...
    return new Promise((resolve, reject) => {
        ...
        const libPath = path_1.default.join(resourcePath, 'lib', '*');
        const optPath = path_1.default.join(resourcePath, 'opt');
        zip.pipe(output);
        [...glob_1.default.sync(optPath), ...glob_1.default.sync(libPath)]
            .filter(folder => fs_extra_1.default.lstatSync(folder).isDirectory())
            .forEach(folder => zip.directory(folder, path_1.default.basename(folder)));
        zip.finalize();
    });
}
async function ensureLayerVersion(context, layerName) {
    ...
    storeResources_1.updateLayerArtifacts(context, layerParameters, { cfnFile: isNewVersion });
}
